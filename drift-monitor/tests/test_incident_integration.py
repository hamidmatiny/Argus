...
from analyzer import DriftAnalyzer, should_raise_incident
...
def test_synthetic_drift_triggers_incident_in_memory():
    analyzer = DriftAnalyzer(baseline_samples=100, window_size=40, min_features_for_incident=2)
    ...
    for i in range(40):
        ...
        # Strong multi-feature shift vs golden baseline.
...
def test_incident_published_to_kafka_topic():
    ...
    analyzer = DriftAnalyzer(baseline_samples=80, window_size=30, min_features_for_incident=2)
    analyzer.seed_synthetic_baseline()
